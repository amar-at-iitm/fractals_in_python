...
import numpy as np
from fractal_verphi import fractal_second_derivative
__all__ = [
    "varphi",
    "phi",
    "psi",
...
def second_divided_difference(x, f, kj):
    """
    Computes the exact 3-point non-uniform second derivative.
    Mathematically identical to the Lagrangian formula.
    """
    # 1. Compute first divided differences (slopes between adjacent points)
    d1 = np.diff(f) / np.diff(x)
    
    # 2. Compute second divided differences across two intervals
    d2 = 2.0 * np.diff(d1) / (x[2:] - x[:-2])
    
    # The result array corresponds to interior indices 1 through len(x)-2.
    # We subtract 1 from kj to map to the correct array index.
    return d2[kj - 1]
